...
//在此块下的每个测试开始时，我们定义了两个集合：
//1。CollectionMarbles-Org1和Org2可以访问此集合
//2。CollectionMarblePrivateDetails-Org2和Org3可以访问此集合
//当用第一个参数“readmarble”调用querychaincode时，它将查询collectionmarbles[1]
//当用第一个参数“readmarbleprivatedetails”调用querychaincode时，它将查询collectionmarbleprivatedetails[2]
...
//升级后，集合将更新如下：
//1。CollectionMarbles-Org1、Org2和Org3可以访问此集合
//2。CollectionMarblePrivateDetails-Org2和Org3可以访问此集合
//更改-org3已添加到收藏大理石中
...
//升级后：
//1。collectionmarbles-org1和org2可以访问此集合，使用readmarble从中读取。
//2。collection marbleprivatedetails-org2和org3可以访问此集合-使用readmarbleprivatedetails从中读取。
...
//由于成员ou onlyou read设置为false，所以org1-user1应该能够
//从CollectionMarblePrivateDetails读取，但找不到
//私有数据。
...
//升级后，集合将更新如下：
//1。CollectionMarbles-成员只读设置为true
//2。collectionmarbleprivatedetails-member_only_read设置为true
//成员身份没有更改，但ORG1-USER1无法读取
