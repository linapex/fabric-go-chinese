...
//
//1。用筏板命令器创建通道，
//2。三节点raft集群上的所有节点都处于同步WRT块中，
//三。raft order处理类型A配置更新并交付
//配置块到对等机。
...
//即使订购方节点不在同一个筏板中，上述功能也可以工作。
//集群；我们需要验证所有三个订购者节点是否同步。
//阻碍。
...
metricsURL := fmt.Sprintf("https://127.0.0.1:%d/metrics“，network.orderport（order，nwo.operationsport））。
logspecURL := fmt.Sprintf("https://127.0.0.1:%d/logspec“，network.orderport（order，nwo.operationsport））。
healthURL := fmt.Sprintf("https://127.0.0.1:%d/healthz“，network.orderport（order，nwo.operationsport））。
