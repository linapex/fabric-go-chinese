...
//但是，因为我们没有为这些对等体提供任何链接代码——服务器不应该
//能够构造描述符。
//只需检查是否返回了适当的错误，什么都不会崩溃。
...
threshold := uint64(3) //使用组织对等点最大高度的3个以内的对等点
...
threshold = 0 //仅使用最高分类账高度的对等方（与使用优先级权重选择器相同）
		acceptablePeers = []string{"p5", "p7", "p9", "p11", "p15"}
		used = make(map[string]struct{})
		for i := 0; i < 10; i++ {
			endorsers, err := mychannel.Endorsers(ccCall("mycc3"), &ledgerHeightFilter{threshold: threshold})
...
//场景六：发现服务发送回一个布局，该布局包含一个没有匹配对等集的组
	svc.On("Discover").Return(&discovery.Response{
		Results: []*discovery.QueryResult{
			{
...
//LedgerHeaghtFilter是一个过滤器，它使用分类帐高度来区分背书人的优先级，尽管它提供了更多
//甚至平衡，而不是简单地按最高分类账高度排序。某些同龄人总是有点
//分类帐的高度比其他人（如领导）高，但我们不应该总是选择领导。
//该过滤器对处于某个块高度阈值内的背书人进行平均处理，并对其进行随机排序。
type ledgerHeightFilter struct {
	threshold uint64
}

//过滤器返回高于配置的分类帐高度阈值的一组随机背书人。
