...
package testprotos //导入“github.com/hyperledger/fabric/common/tools/prototator/testprotos”
...
//simplemsg旨在测试所有三种类型的消息字段：plain、map和
//切片由Protolator工具处理
...
//staticallyopaquemsg用于测试静态不透明消息组件
//所有字段都以静态方式封送到nestedmsg类型
...
//variablyopaquemsg用于测试静态不透明消息组件
//不透明类型由不透明类型确定
...
//dynamicmsg用于测试动态消息组件
//应用于contextlessmsg的动态包装由
//动态型
...
//可取消标记的DeepFields包含定义为不透明的字段，但将
//
