...
//需要此依赖项，因为索引“indexableattrxid”用于检测重复的txid
//结果在其他两个索引中重用。理想情况下，这三个索引应该合并为一个
//出于效率目的-[FAB-10587]
...
if txoffset.isDuplicate { //不要覆盖索引-FAB-8557中的txid条目
...
//index4-store blocknumtrannum将用于查询历史数据
...
func (index *blockIndex) markDuplicateTxids(blockIdxInfo *blockIdxInfo) error {
	...
	for _, txIdxInfo := range blockIdxInfo.txOffsets {
		txid := txIdxInfo.txID
if uniqueTxids[txid] { //txid与块中以前的tx重复
			txIdxInfo.isDuplicate = true
			continue
		}

		loc, err := index.getTxLoc(txid)
if loc != nil { //txid与索引中以前的tx重复
			txIdxInfo.isDuplicate = true
			continue
		}
