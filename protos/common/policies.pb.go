...
//SignaturePolicyInvelope包装了一个SignaturePolicy，并包含了一个用于将来增强的版本
...
//SignaturePolicy是一个递归消息结构，它定义了一个轻量级DSL用于描述
//比“正是这个签名”更复杂的策略。noutof运算符就足够了
//表达和，以及或，当然也包括以下m个策略中的n个
//signedBy表示签名来自由受信任方签名的有效证书
//字节中指定的权限。这将是自签名证书的证书本身
//将成为更传统证书的CA
...
//implicitMetapolicy是一种策略类型，它取决于配置的层次结构性质。
//它是隐式的，因为规则是根据子策略的数量隐式生成的
//它是元的，因为它只依赖于其他策略的结果
//评估时，此策略将迭代所有直接子组，并检索策略
//名为Sub-U策略的，评估集合并应用规则。
//例如，使用4个子组和策略名称“foo”，implicitmetapolicy检索
//每个子组，为每个子组检索策略“foo”，对其进行评估，如果有
//满足就足够了，全部需要4个签名，大多数需要3个签名。
