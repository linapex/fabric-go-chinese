...
//configendevelope设计用于包含没有依赖关系的链的所有配置
//在以前的配置事务上。
//
//采用以下方案生成：
//1。检索现有配置
//2。注意要修改的配置属性（configValue、configPolicy、configGroup）
//三。将任何中间配置组添加到configupdate.read_集（稀疏地）
//4。向configupdate.read_集（稀疏地）添加任何其他所需依赖项
//5。修改配置属性，将每个版本增加1，并在configupdate.write\u集中设置它们。
//注意：任何未修改但指定的元素都应该已经在读取集中，因此可以稀疏地指定。
//6。创建configupdate消息并将其封送到configupdateedevelope.update中，并对所需签名进行编码。
//a）每个签名都是configSignature类型
//b）configSignature签名位于签名头和configUpdate字节（包括chainheader）的串联之上。
//5。提交新配置以便在提交者签名的信封中订购
//a）信封有效载荷的数据设置为已编组的configendevelope
//b）信封有效负载具有header.type.config_update类型的头
//
//配置管理器将验证：
//1。读取版本中存在读取集中的所有项
//2。写入\集中的所有项目的版本与读取\集中的版本不同，或不在读取\集中的所有项目已根据其mod \策略进行了适当的签名。
//三。新配置满足配置模式
...
//configupdate用于提交config的子集，并使排序器应用于config
//它总是在configupdatenedevelope中提交，它允许添加签名
//导致新的总体配置。更新应用如下：
//1。读取集中所有元素的版本将根据现有配置中的版本进行验证。
//如果读取版本不匹配，则配置更新将失败并被拒绝。
//2。写入\集中与读取\集版本相同的任何元素都将被忽略。
//三。将收集写入集中每个剩余元素的相应mod_策略。
//4。根据configupdateedevelope的签名检查每个策略，任何未验证的都将被拒绝。
//5。写入集应用于配置，configgroupschema验证更新是否合法。
