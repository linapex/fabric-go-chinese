...
//lastconfig是在最后一个配置块元数据索引中编码的元数据消息的编码值。
...
//header是一个通用的重播预防和标识消息，包含在签名的有效负载中
...
//生成此头的epoch，其中epoch是根据块高度定义的。
//产生响应的时间。此字段标识
//逻辑时间窗。只有在以下情况下，同行才会接受建议回复：
//有两个条件：
//1。消息中指定的epoch是当前epoch
//2。在这个时代，这个信息只出现过一次（也就是说，它没有
//重播）
...
//消息的创建者，已封送的msp.serialididEntity
	Creator []byte `protobuf:"bytes,1,opt,name=creator,proto3" json:"creator,omitempty"`
//只能使用一次的任意数字。可用于检测重播攻击。
...
//这是最终确定的块结构，将在订购方和对等方之间共享
//请注意，blockheader链接到上一个blockheader，并嵌入blockdata散列
//在blockheader中。这使得数据包含在哈希中变得自然而明显，但是
//元数据不是。
type Block struct {
