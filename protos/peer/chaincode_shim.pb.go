...
//来源：peer/chaincode\u shim.proto
...
//由链码发出的事件。仅与init或invoke一起使用。
//然后存储此事件（当前）
//使用block.nonhashdata.TransactionResult
...
//putstate是chaincodemessage的有效负载。它包含一个键和值
//需要写入事务的写入集。如果集合是
//指定后，密钥和值将写入事务的私有
//写集。
...
//delstate是chaincodemessage的有效负载。它包含一把钥匙，
//需要在事务的写入集中记录为删除操作。
//如果指定了集合，则需要在
//事务的私有写入设置为删除操作。
...
//GetStateByRange是链式代码消息的有效负载。它包含一个开始键和
//执行范围查询所需的结束键。如果指定了集合，
//需要对私有数据执行范围查询。元数据保留
//querymetadata的字节表示形式。
...
//GetQueryResult是chaincodeMessage的有效负载。它包含一个查询
//基础状态数据库支持的窗体中的字符串。
//如果指定了集合，则需要在
//私有数据。元数据包含querymetadata的字节表示形式。
...
//QueryMetadata是GetStateByRange和GetQueryResult的元数据。
//它包含一个页面大小，表示要提取的记录数。
//还有书签。
...
//GetHistoryForkey是链式代码消息的有效负载。它包含一把钥匙
//需要检索历史值。
...
//对等端作为getStateByRange的结果返回queryResponse，
//GetQueryResult和GetHistoryForkey。里面有一堆记录
//结果字段，指示是否需要从中提取更多结果的标志。
//中的对等机具有更多字段、ID字段中的事务ID和QueryResponseMetadata
//在元数据字段中。
...
//QueryResponseMetadata是QueryResponse的元数据。它包含一个计数
//它表示从分类帐和书签中提取的记录数。
