...
//chaincodeid包含deploy事务指定的路径
//创建了它以及由
//路径的系统。从用户级别（例如，cli、rest api等）
//Deploy事务应提供路径和其他请求
//应提供哈希代码。另一个值将被忽略。
//在内部，结构可以包含这两个值。例如，
//将在第一次使用路径生成时设置哈希代码
...
//携带chaincode函数及其参数。
//unmashaljson in transaction.go将基于字符串的rest/json输入转换为
//基于[]字节的当前chaincodeinput结构。
