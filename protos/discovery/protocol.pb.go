...
//来源：discovery/protocol.proto

package discovery //导入“github.com/hyperledger/fabric/protos/discovery”
...
//SignedRequest在有效负载字段中包含一个序列化请求
//还有签名。
//用于验证签名的标识
//可以从authinfo类型的身份验证字段中提取
//在对请求进行反序列化之后。
...
//query result包含给定查询的结果。
//通过查询结果的索引可以推断出相应的查询
//它的封闭响应消息。
//查询结果的顺序与查询在其封闭请求中的顺序相同。
...
//peermembershipquery请求peermembershipresult。
//筛选字段可以按顺序可选地填充。
//对于要根据筛选的对等成员身份
//安装在对等端和集合上的链码
//访问控制策略。
...
//chaincodequery为给定的请求chaincodequeryresults
//链码调用列表。
//每个调用都是一个单独的调用，并且认可策略
//对每个给定的兴趣进行独立评估。
...
//背书描述符包含有关可以使用哪些对等方的信息
//要求背书，使背书政策得以履行。
//下面是如何计算一组对等点以向其请求背书，给定一个背书描述符：
//让e:g->p成为将一个组映射到一组对等方的“背书人”字段。
//注意，在G组上应用e会产生一组对等体。
//1）从给定的布局中选择布局l:g->n。
//L是布局的“按组的数量”字段，它将组映射为整数。
//2）R=（一组空的对等点）
//3）对于布局L中的每组G，计算n=l（g）
//3.1）将p_g表示为一组n个随机对等点p0，p1，…p_n从e（g）中选择
//3.2）r=r u p_g（加p_g至r）
//4）对等方r是客户要求背书的对等方。
