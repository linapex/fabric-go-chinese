...
package gossip //导入“github.com/hyperledger/fabric/protos/八卦”
...
func _Secret_OneofUnmarshaler(msg proto.Message, tag, wire int, b *proto.Buffer) (bool, error) {
	m := msg.(*Secret)
	switch tag {
case 1: //content.internalendpoint内部端点
...
case 19: //content.state响应
		...
		return true, err
case 20: //content.leadership_msg（内容领导）
		if wire != proto.WireBytes {
			return true, proto.ErrInternalBadWireType
		}
...
//StateInfoPullRequest用于获取StateInfoSnapshot
//从远程对等机
...
//Connestablish是用于八卦握手的信息
//每当一个对等机连接到另一个对等机时，它就会握手
//通过发送这个消息来证明它的身份
...
//DataDigest是从接收器对等端发送的消息
//到初始化程序对等端，并包含其具有的数据项
...
//包含private的privatedatamessage消息
//向分布式一次事务发送数据信息
//已背书
...
//用于请求的RemotePrivateDataRequest消息
//缺少private rwset
...
//要在专用服务器上响应的RemotePrivateData消息
//数据复制请求
