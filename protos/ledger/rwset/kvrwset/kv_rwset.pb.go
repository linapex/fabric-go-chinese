...
//资料来源：ledger/rwset/kv rwset/kvrwset.proto

package kvrwset //导入“github.com/hyperledger/fabric/protos/ledger/rwset/kvrwset”
...
//kvrwset封装了对kv或文档数据模型进行操作的链代码的读写集。
//此结构用于公共数据和私有数据
type KVRWSet struct {
...
//kvreadhash与kvread在精神上类似。但是，它捕获键的散列而不是键本身
//版本暂时保持不变。但是，如果版本还需要隐私保护，则需要
//版本的哈希，因此是“bytes”类型
...
//版本封装密钥的版本
//已提交密钥的版本被维护为提交该密钥的事务的高度。
//高度被表示为一个元组<blocknum，txnum>，其中txnum是事务的位置。
//（从0开始）在块内
type Version struct {
...
//rangequeryinfo封装模拟期间事务执行的范围查询的详细信息。
//这有助于通过在验证是否有任何新项目时进行变量化来保护事务不受幻象读取的影响。
//在事务模拟和验证之间的给定范围内提交
//（除了定期检查现有项目的更新/删除之外）。
//readinfo字段包含kvards（对于范围查询读取的项目）或merkle树哈希
//如果kvreads超过预先配置的数字
...
//queryreadsmerklesummary封装了用于queryreads的merkle树哈希
//这允许在查询结果出现时减小rwset的大小。
//通过存储某些哈希而不是实际结果。
//maxdegree字段是指树中任何级别的子级的最大数目。
//maxlevel字段包含节点数小于maxdegree的最低级别（从叶级开始）
