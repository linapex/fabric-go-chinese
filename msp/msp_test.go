...
//将一个或多个mspprincipals合并为类型的mspprincipal
//mspprincipal_组合。
...
//必需，因为从localmsp中删除cert或msp选项会导致并行测试失败
var localMspBad MSP
...
//测试数据/MSPID
//1）一个密钥和一个签名证书（用于填充默认签名标识），其中证书具有高位签名。
