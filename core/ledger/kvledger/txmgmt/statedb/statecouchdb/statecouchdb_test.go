...
//对于couchdb，byteskeysupported应为false
...
//validateKeyValue应为包含保留字段之一的JSON值返回错误。
//在顶层
...
//对于包含保留字段之一的JSON值，validateKeyValue不应返回错误。
//如果不是在顶层
	for _, reservedField := range reservedFields {
...
//validateKeyValue应返回以下划线开头的键的错误
...
//testinvalidjsonfields测试无效的json字段
...
//查询命名空间“ns2”，索引仅在“ns1”中创建。这将返回一个错误。
...
//testpaginatedqueryvalidation使用分页测试查询
...
//场景2：更新lscc命名空间中的现有密钥。请注意
//应更新lsccstatecache中的键
	batch = statedb.NewUpdateBatch()
...
//场景3：在lscc命名空间中添加lsccachesize键数。
//读取lscc命名空间中的所有键以使缓存满。这是为了
//测试驱逐。
	batch = statedb.NewUpdateBatch()
...
//db中的键按此顺序排列
//键1，键2，键3，_设计/索引资产名称，_设计/索引资产值，键1，键2，键3
//查询不同范围并验证结果
