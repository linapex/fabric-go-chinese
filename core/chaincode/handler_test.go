...
fakeCollectionStore.HasReadAccessReturns(false, nil) //到
//确保使用访问缓存
				})
...
Expect(txParams.TXSimulator).To(BeIdenticalTo(newTxSimulator)) //相同的实例，而不仅仅是相等的
			})
			...
			It("provides the new history query executor in the context used for execution", func() {
				...
				Expect(fakeInvoker.InvokeCallCount()).To(Equal(1))
				txParams, _, _ := fakeInvoker.InvokeArgsForCall(0)
Expect(txParams.HistoryQueryExecutor).To(BeIdenticalTo(newHistoryQueryExecutor)) //相同的实例，而不仅仅是相等的
			})
	...
	Describe("ProcessStream", func() {
		...
		Context("when an async error is sent", func() {
			...
			It("returns an error", func() {
				errChan := make(chan error, 1)
				go func() { errChan <- handler.ProcessStream(fakeChatStream) }()
Eventually(fakeChatStream.RecvCallCount).ShouldNot(Equal(0))                               //等待循环开始
handler.Execute(&ccprovider.TransactionParams{}, cccid, incomingMessage, time.Millisecond) //强制异步错误
