...
//testinstall使用各种输入测试安装功能
func TestInstall(t *testing.T) {
//初始化Inturn初始化内部组件的Ledgermgmt（如此测试所依赖的ccEventmgmt）
...
//constructdeploymentspec将depspec放在fs上。这应该会成功
...
//由于禁用了privatechanneldata，由于存在
//stub.args中的collectionconfigbytes
...
//由于已启用privateChannelData且collectionConfigBytes无效，因此出现以下错误
//需要消息。
...
//由于已启用privateChannelData且collectionConfigBytes有效，因此不应出现任何错误。
	testDeploy(t, "example02", "1.0", path, false, false, true, "", scc, stub, ccpBytes)
//应包含两个条目：一个用于chaincodedata，另一个用于collectionconfigbytes
...
//由于已启用privateChannelData，collectionConfigBytes为零，因此不应出现任何错误。
	testDeploy(t, "example02", "1.0", path, false, false, true, "", scc, stub, []byte("nil"))
//只应包含chaincodedata的条目。由于collectionconfigbytes为零，因此
//被忽略
	assert.Equal(t, 1, len(stub.State))
	_, ok = stub.State["example02"]
	assert.Equal(t, true, ok)
...
//仅当CollectionConfigBytes为零时，才应包含链码数据的条目
	assert.Equal(t, 1, len(stub.State))
	_, ok := stub.State["example02"]
	assert.Equal(t, true, ok)
...
//应包含两个条目：一个用于chaincodedata，另一个用于collectionconfigbytes
//因为v12validation已启用。只有在v12validation中，才允许集合升级。
//请注意，v12validation将替换为collectionupgrade功能。
	assert.Equal(t, 2, len(stub.State))
	_, ok = stub.State["example02"]
	assert.Equal(t, true, ok)
	actualccpBytes, ok := stub.State["example02~collection"]
